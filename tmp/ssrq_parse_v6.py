...
def extract_years(text):
    ...
    while i < len(text):
        if '0' <= text[i] <= '9':
            ...
            while end < len(text) and '0' <= text[end] <= '9' and end < i + 4:
                end += 1
            if end - i == 4:
                y = int(text[i:end])
                if 1200 <= y <= 2099:
...
def process_person(eid, uri, raw):
    ...
    if not std_name and fn0 and sn0:
        std_name = fn0 + ' ' + sn0
    elif not std_name and sn0:
        std_name = sn0
    oi = []
    for line in raw.split('\n'):
...
def main():
    ...
    init_db(conn)
    for t in ("persons", "orgs", "name_index"):
    ...
    with open(TTL, 'r', encoding='utf-8', errors='replace') as f:
        for line in f:
            ...
            if not cur_type:
                # Waiting for URL-only line like "<http://ssrq...#org000000>"
                h = line.find(MARK)
                if h >= 0:
                    e = line.find('>', h)
                    if e >= 0:
                        # Check this line is ONLY the URL (no other content)
                        line_stripped = line.strip()
                        url_only = (line_stripped.startswith('<') and
                                    line_stripped.rstrip().endswith('>') and
                                    line_stripped.count(' ') == 0)
                        if url_only:
                            cur_uri = line_stripped
                            pound = line.find('#', h)
                            if pound >= 0 and pound < e:
                                entity = line[pound+1:e]
                                first_digit = 0
                                while first_digit < len(entity) and not entity[first_digit].isdigit():
                                    first_digit += 1
                                cur_type = entity[:first_digit]
                                cur_id = entity[first_digit:]
                        # else: URL is part of a predicate like "pers:org_id <...>" — ignore
            else:
                buf.append(line)
                # Entity ends when we see a line ending with ".\n" or ".\r\n"
                ls = line.rstrip()
                if ls.endswith('.'):
                    # Check there's a closing quote before the dot
                    q = line.find('"')
                    d = line.rfind('.', q)
                    if q >= 0 and d > q:
                        raw = ''.join(buf)
                        if cur_type == 'per':
                            ...
                            orgs.append((cur_id, cur_uri, 'org', '', '', '', '', ''))
                        cur_type = None
                        cur_id = None
                        buf = []

    print("  persons: " + str(len(persons)) + "  orgs: " + str(len(orgs)))
    ...
    if orgs:
        ph = ','.join(['?' for _ in co])
        conn.executemany('INSERT OR REPLACE INTO orgs VALUES (' + ph + ')', orgs)
        conn.commit()
        print("  inserted " + str(len(orgs)) + " orgs")
    ...
    for row in conn.execute('SELECT id, std_name, forename, surname, orig_names, std_names, label FROM persons'):
        ...
        if label:
            add(label.split('@')[0])
        for n in json.loads(on_js or '[]'):
            add(n, 1)
        for n in json.loads(sn_js or '[]'):
            add(n)
        if len(ni) >= 2000:
            conn.executemany('INSERT OR IGNORE INTO name_index VALUES(?,?,?)', ni)
            ni = []
    if ni:
        conn.executemany('INSERT OR IGNORE INTO name_index VALUES(?,?,?)', ni)
    conn.commit()
