...
def _voyant_url(text: str, corpus_name: str) -> str:
    """Erstellt Voyant-Tools-Link mit eingebettetem Korpus (AH-52, closes #29)."""
    api_base = config.VOYANT_API_URL.rstrip("/")
    corpus_id = corpus_name.lower().replace(" ", "_")
    snippet = text[:50000]

    # Try form-encoded POST — local Voyant server accepts /upload, /corpus, or /Corpus
    for endpoint in ["/upload", "/corpus"]:
        try:
            ...
            with urllib.request.urlopen(req, timeout=30) as resp:
                ...
                if resp.status == 200:
                    try:
                        ...
                        # Fallback: return direct URL with corpus ID as query param
                        return f"{api_base}/?corpus={corpus_id}"
        except Exception as e:
            continue

    # Final fallback: direct link
    logger.warning("[Agent D] Voyant-API fehlgeschlagen; Fallback-URL verwendet.")
    return f"{api_base}/?corpus={corpus_id}"
